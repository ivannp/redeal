...
class make_build(build_py, object):
    def run(self):
        ...
        if sys.platform.startswith("linux") or sys.platform == "darwin":
            prebuilt_path = os.path.join(base_dir, 'prebuilt', 'libdds.so')
            if os.path.isfile(prebuilt_path):
                libdds_path = prebuilt_path
                shutil.copy(libdds_path, os.path.join(self.build_lib, "redeal", "libdds.so"))
            else:
                self._build_dds(base_dir)
                libdds_path = os.path.join(base_dir, "dds", "src", "libdds.so")
                shutil.move(libdds_path, os.path.join(self.build_lib, "redeal", "libdds.so"))


    def _build_dds(self, base_dir):
        orig_dir = os.getcwd()
        try:
